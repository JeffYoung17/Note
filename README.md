...
（1）为了实现进程同步，例如生产者消费者的经典例子：  
仅利用缓冲区的空闲个数作为判断依据是不够的。因此提出对应这个缓冲区的信号量，进程根据信号量来判读是否要阻塞等待或者唤醒其他的等待进程。  
（2）为了确保共享缓冲区的对应信号量是正确的，因为多个进程都会去修改信号量，需要保护这个信号量。否则进程A修改了信号量，要执行后面的代码时，发生了阻塞调度并切换到进程B，而进程B也修改了信号量，这样就有问题了。  
需要给信号量上锁。  
（3）临界区：是一段代码，这段代码一次只允许一个进程进入；  
轮换法，标记法，非对称标记法，Peterson算法，面包店算法；  
硬件原子指令法：当一个进程进入临界区时，关闭中断，这样就不会发生其他进程的调度和切换。  
...
用户程序编译完得到的指令中的地址为逻辑地址，并非内存的实际物理地址；  

需要重定位功能：即将程序指令中的逻辑地址 --> 定位成 内存中的物理地址；  

根据完成重定位的时间：  

```
编译时重定位：程序只能放在内存的固定位置；
载入时重定位：可以换位置，但一旦程序载入内存后，就不能换位置了；
运行时重定位：交换的概念，为了充分利用内存，当前需要睡眠的进程可能放入磁盘，当前等待的进程可能从磁盘拿到内存，即进程的物理地址不固定，需要在运行时确定。
```

用户程序的分段：  

用户程序不是一起载入内存，而是被分成多个段。  

#### L21 内存分区与分页

用户程序载入内存，系统需要给他分配位置。  

朴素思想：固定分区，但是不同程序需要的空间不一样，所以应该采用可变分区；  

可变分区的管理：空闲分区表（起始地址，空闲长度），已分配分区表（起始地址，长度，段的标记）。  

当有段提出请求：请给我分配内存吧！现有的思想可能会造成，内存碎片的现象；  

将内存分页：以页为单位进行分配，页表（页号，页框号，权限）。  
